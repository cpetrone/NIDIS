...
%           1- X values (distance)
%           2- Y values (greyvalues)
%           3- error values on Y (standard error of the mean)

%  Output folder:
%      Data on fitting parameters: y0, erf_par, x0, Dt. 
...
function [fitresult, gof] = createfit_MAC(distance,greyvalues,errors)
...
xdata = distance;
ydata = greyvalues;
x0_start = (max(xdata) + min(xdata)) / 2;
...
y_last = length(ydata);
if ydata(y_last) < ydata(1) % Complementary error function shape - ERFC 
    ft = fittype('y0+1-(erf_par*(erf((x-x0)/Dt)))',...
    'dependent',{'y'},'independent',{'x'},...
    'coefficients',{'y0', 'erf_par', 'x0', 'Dt'});
elseif ydata(y_last) > ydata(1) % Error function shape - ERF
     ft = fittype('y0+(erf_par*(erf((x-x0)/Dt)))',...
    'dependent',{'y'},'independent',{'x'},...
    'coefficients',{'y0', 'erf_par', 'x0', 'Dt'});
end
opts = fitoptions( ft );
...
opts.StartPoint = [y0_start erf_par_start x0_start Dt_start];
...
dir_number = 1;
directory_name = strcat(reference_name, sprintf('_fit_%d', dir_number));
...
if ydata(y_last) < ydata(1) % ERFC 
    yfit = param_values(1)+1-(param_values(2)*(erf((xdata-param_values(3))/param_values(4))));
elseif ydata(y_last) > ydata(1) % ERF 
    yfit = param_values(1)+(param_values(2)*(erf((xdata-param_values(3))/param_values(4))));
end
y0_data = horzcat(param_names(1),param_values(1),conf_int(:,1));
erf_par_data = horzcat(param_names(2),param_values(2),conf_int(:,2));
...
save(workspace_full_name, 'param_values', 'yfit', 'gof', 'x0_data', 'y0_data', 'erf_par_data', 'Dt_data');
...
% Calculate residence time and its error using different methods.
...
elseif yfit_inserted_Dt_msg_answer == 'y'
    ...
    Dt_inserted_Dt = cell2mat(Dt_data(2));
    param_values_inserted_Dt = param_values;
    param_values_inserted_Dt(4) = Dt_inserted_Dt;
    if ydata(y_last) < ydata(1) % ERFC 
        yfit_inserted_Dt = param_values_inserted_Dt(1)+1-(param_values_inserted_Dt(2)*(erf((xdata-param_values_inserted_Dt(3))/param_values_inserted_Dt(4))));
    elseif ydata(y_last) > ydata(1) % ERF 
        yfit_inserted_Dt = param_values_inserted_Dt(1)+(param_values_inserted_Dt(2)*(erf((xdata-param_values_inserted_Dt(3))/param_values_inserted_Dt(4))));
    end
